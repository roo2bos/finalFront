  ...
  return (
    <>
      <form className='authForm' onSubmit={handleSubmit(onSubmit, onError)}>
        ...
        />
        {errors.username && <span className='authSpan' role="alert">{errors.username.message}</span>}
        ...
        {userId && <span className='text-blue-500 text-xs'> {userId}은(는) 사용 가능한 아이디입니다.</span>}
        {errors.userId && <span className='authSpan' role="alert">{errors.userId.message}</span>}
        ...
        {errors.confirmPassword && (
          <span className='authSpan' role="alert">{errors.confirmPassword.message}</span>
        )}

<div className="flex items-center align-center mt-10 mb-2">
<label className='authLable' htmlFor="agree">위에 약관에 동의하십니까?</label>
  <input
  ...
  />
   {errors.agree && <span className='authSpan mt-2' role="alert">{errors.agree.message}</span>}
</div>

       

        <button className='authInput'  type="submit">가입하기</button>
      </form>
    </>
  );
}
